...
import client, {OpenAPIClientAxios} from 'openapi-client-axios'
...
export class HttpClient {
  ...
  constructor(config: HttpClientConfig, openApiSpec: OpenAPIV3.Document | OpenAPIV3_1.Document) {
    ...
    this.client = new (client.default ?? OpenAPIClientAxios)({
  ...
  private isFileUploadParameter(operation: OpenAPIV3.OperationObject): string[] {
  ...
  private async prepareFileUpload(operation: OpenAPIV3.OperationObject, params: Record<string, any>): Promise<FormData | null> {
    const fileParams = this.isFileUploadParameter(operation)
